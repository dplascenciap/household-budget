...
export default function Dashboard({ user }) {
  ...
  // Rent-filtered values for summary cards and charts
  ...
  return (
    <>
      ...
      {/* Rent toggle */}
      <div className="rent-toggle-bar">
        <button
          className={`rent-toggle-btn${hideRent ? ' active' : ''}`}
          onClick={() => setHideRent(h => !h)}
        >
          {hideRent ? '🏠 Rent hidden' : '🏠 Hide rent'}
        </button>
        {hideRent && (
          <span className="rent-toggle-note">
            Showing discretionary spending only
          </span>
        )}
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid-2">
            <ChartPanel expenses={displayExpenses} month={month} totalBudget={displayBudget} />
